...
C-k : Delete pattern
C-o : Persistent Action (Execute action with helm session kept)
...
`helm-execute-persistent-action' called with a prefix arg (C-u C-o) or with its specific command
...
- Use `C-u C-o' to watch an image.

- `C-o' on a filename will expand in helm-buffer to this filename.
  Second hit on `C-o' will display buffer filename.
  Third hit on `C-o' will kill buffer filename.
  NOTE: `C-u C-o' will display buffer directly.
...
\\[helm-grep-run-persistent-action]\t\t->Run persistent action (Same as `C-o').
...
You can get help on any command with persistent action (C-o).
